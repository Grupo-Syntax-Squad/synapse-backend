...
import redis
from src.database.redis import get_redis_dependency
...
from src.schemas.auth import LoginForm, ForgotPasswordRequest, ResetPasswordRequest
from src.schemas.basic_response import BasicResponse
from src.modules.password_reset import (
...
def forgot_password(
    ...
    service = RequestPasswordResetService(session, redis_client, request)
    result = service.execute()

    if result:
        user, token = result
        email_service = SendResetEmailService(user, token)
        background_tasks.add_task(email_service.execute)

    return BasicResponse(
        message="Se um usuário com esse e-mail estiver cadastrado, um link de recuperação foi enviado."
    )
...
def reset_password(
    ...
    service = ResetPasswordService(session, redis_client, request)
    service.execute()

    return BasicResponse(message="Senha alterada com sucesso.")
