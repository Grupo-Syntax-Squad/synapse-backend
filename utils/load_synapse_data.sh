...
psql -U postgres -d "$DB_NAME" -f ./setup_tables.sql

psql -U postgres -d "$DB_NAME" -c "\COPY clientes(cod_cliente) FROM '$CLIENTES_FILE' WITH (FORMAT csv, HEADER true, DELIMITER '|')"
psql -U postgres -d "$DB_NAME" -c "\COPY estoque FROM '$ESTOQUE_FILE' WITH (FORMAT csv, HEADER true, DELIMITER '|')"
psql -U postgres -d "$DB_NAME" -c "\COPY faturamento FROM '$FATURAMENTO_FILE' WITH (FORMAT csv, HEADER true, DELIMITER '|')"
